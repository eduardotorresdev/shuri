 ...
 * Every method receives the `CollectionSchema` it operates on so an adapter can translate
 * the engine-agnostic `Query` AST into its own native query language.
 */
export interface StoreAdapter {
  ...
  update(collection: CollectionSchema, id: RecordId, data: RecordInput): Promise<StoreRecord>;
